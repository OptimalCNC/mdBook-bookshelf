...
`additional-js` lists. Their contents are stable for a given
`mdbook-bookshelf` version, and the build does not rewrite an existing asset
whose generated bytes are already current. mdBook copies them into each book
