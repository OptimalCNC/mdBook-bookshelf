...
#[test]
fn serve_cli_serves_root_authored_page_and_search_index() {
    ...
    assert!(root.status_line.starts_with("HTTP/1.1 200"));
    assert!(root.body.contains("Bookshelf"));
    assert!(root.body.contains("href=\"docs/index.html\""));
    assert!(root
        .body
        .contains("href=\"modules/parser/docs/index.html\""));
    assert!(root.body.contains("href=\"modules/ui/docs/index.html\""));

    let onboarding = http_get(&addr, "/docs/onboarding.html");
    assert!(onboarding.status_line.starts_with("HTTP/1.1 200"));
    assert!(onboarding.body.contains("Onboarding"));
    assert!(onboarding.body.contains("Bookshelf"));

    let search = http_get(&addr, "/searchindex.json");
    assert!(search.status_line.starts_with("HTTP/1.1 200"));
    assert!(search.body.contains("\"book_label\":\"Example Core\""));
    assert!(search.body.contains("\"href\":\"docs/onboarding.html\""));
