...
#[test]
fn writes_only_the_canonical_root_bookshelf_page() {
    ...
    let output_dir = temp_root().join(unique_dir_name("bookshelf-root-html-invariants"));

    render_bookshelf_root_page(&output_dir, &site_model, &render_manifest)
        .expect("render should succeed");

    let files = collect_files(&output_dir);
    assert_eq!(files, vec![PathBuf::from("index.html")]);

    let html = fs::read_to_string(output_dir.join("index.html"))
        .expect("rendered html should be readable");
    assert!(html.contains("href=\"docs/index.html\""));
    assert!(!html.contains("href=\"/\""));
    assert!(!html.contains("href=\"index.html\""));
    assert!(!html.contains("href=\"bookshelf/\""));
    assert!(!output_dir.join("bookshelf/index.html").exists());
    assert!(!output_dir.join("docs/index.html").exists());
    assert!(!output_dir.join("modules/parser/docs/index.html").exists());
}
